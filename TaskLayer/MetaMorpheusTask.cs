...
namespace TaskLayer
{
    ...
    public abstract class MetaMorpheusTask
    {
        ...
        protected List<Protein> LoadProteins(string taskId, List<DbForTask> dbFilenameList, bool searchTarget, DecoyType decoyType, List<string> localizeableModificationTypes, CommonParameters commonParameters)
        {
            ...
            proteinList = AddTruncatedProteoforms(proteinList);
        ...
        private List<Protein> AddTruncatedProteoforms(List<Protein> proteinList)
        {
            List<Protein> newProteinList = new();
            
            foreach (Protein protein in proteinList)
            {
                newProteinList.Add(protein);
                if(protein.BaseSequence.Length > 20)
                {
                    for (int i = 1; i <= 8; i++)
                    {
                        newProteinList.Add(AddNminusItruncatedForm(protein, i));
                        newProteinList.Add(AddCminusItruncatedForm(protein, i));
                    }
                }
            }
        ...
        private Protein AddCminusItruncatedForm(Protein protein, int i)
        {
            Protein p = new Protein(protein.BaseSequence.Substring(0,protein.BaseSequence.Length - i), protein.Accession + "-C" + i, protein.Organism, protein.GeneNames.ToList(), protein.OneBasedPossibleLocalizedModifications, protein.ProteolysisProducts.ToList(), protein.Name + "-C" + i, protein.FullName,
                protein.IsDecoy, protein.IsContaminant, protein.DatabaseReferences.ToList(), protein.SequenceVariations.ToList(), protein.AppliedSequenceVariations, protein.SampleNameForVariants, protein.DisulfideBonds.ToList(),
                protein.SpliceSites.ToList(), protein.DatabaseFilePath);
            return p;
        }

        private Protein AddNminusItruncatedForm(Protein protein, int i)
        {
            Protein p = new Protein(protein.BaseSequence.Substring(i, protein.BaseSequence.Length - i), protein.Accession + "-N" + i, protein.Organism, protein.GeneNames.ToList(), protein.OneBasedPossibleLocalizedModifications, protein.ProteolysisProducts.ToList(), protein.Name + "-N" + i, protein.FullName,
                protein.IsDecoy, protein.IsContaminant, protein.DatabaseReferences.ToList(), protein.SequenceVariations.ToList(), protein.AppliedSequenceVariations, protein.SampleNameForVariants, protein.DisulfideBonds.ToList(),
                protein.SpliceSites.ToList(), protein.DatabaseFilePath);
            return p;
        }
